...
func group(session []domain.Session) map[int64][]domain.Session {
	buckets := make(map[int64][]domain.Session)
...
func repositories(tempSessions []domain.Session) []domain.Repository {
	...
	// and filename.
	repositoryFileMap := make(map[string]map[string]*domain.DailyFile)
	for _, tempSession := range tempSessions {
		for _, file := range tempSession.Files {
			...
			if _, ok := repositoryFileMap[file.Repository]; !ok {
				repositoryFileMap[file.Repository] = make(map[string]*domain.DailyFile)
			}
			...
			if _, ok := repositoryFileMap[file.Repository][file.Name]; !ok {
				repositoryFileMap[file.Repository][file.Name] = &domain.DailyFile{
					Name:       file.Name,
					Path:       file.Path,
...
// aggregated sessions.
func sessions(buckets map[int64][]domain.Session) []domain.AggregatedSession {
	sessions := make([]domain.AggregatedSession, 0)
...
func TemporarySessions() []domain.AggregatedSession {

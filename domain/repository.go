...
// Repository represents a git repository. A coding session might open files
// across any number of repos. The files of the coding session are later
...
// repositoryPathFile takes a slice of coding sessions and generates a map. In
// this map, the key is the repository name, and the value is another map
// wherein the key is the relative file path within the repository, and the
// value is the file itself. The files are merged if any of the sessions has
...
func repositoryPathFile(sessions []Session) map[string]map[string]AggregatedFile {
	...
	for _, session := range sessions {
      ...
      // Check if it is the first time we're seeing a repository with this
      // name. In that case, we'll initialize the map
			if _, ok := repoPathFile[file.Repository]; !ok {
				repoPathFile[file.Repository] = make(map[string]AggregatedFile)
			}

      // Check if it's the first time we're seeing this file in this
      // repository. If it's not the first time, we'll merge them
